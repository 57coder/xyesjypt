...
<html>
  ...
  <body class="body" leftmargin="0" rightmargin="0">
    <form action="order?type=orderReply" method="post">
     <table cellspacing="1" cellpadding="0" width="100%" border="0" bgcolor="#F7F3F7">
          <tr height="26">
              <td class="blackTitle" align="center" width="40%">&nbsp;物品名称</td>
              <td class="blackTitle" align="center" width="20%">图片</td>
              <td class="blackTitle" align="center" width="40%">评价</td>
              <td class="blackTitle" align="center" width="40%">回复</td>
          </tr>
		  <c:forEach items="${requestScope.pingItemList}" var="pingItem">
		  <tr class="text" align="center" bgcolor="#FFFFFF">
			  <td>${pingItem.goods.mingcheng}</td>
			  <td><img src="<%=path %>${pingItem.goods.fujian}" width="50px" height="50px"/>
			     
			  </td>
			   <td>${pingItem.info}<br/>${pingItem.adddate}</td>
			  <td><textarea name="txt${pingItem.id}" cols="25" rows="5"></textarea>
		   ...
		   <tr height="26">
              <td class="blackTitle" align="center" colspan="4">
                 <input type="hidden" name="pingid" value="${requestScope.pingid}"/>
