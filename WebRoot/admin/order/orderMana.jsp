...
function orderDetail(order_id) {
	var url = "<%=path%>/buy?type=orderDetailHou&order_id=" + order_id;
	var n = "";
	var w = "600px";
	var h = "500px";
...
function orderReply(order_id) {
	var url = "<%=path%>/buy?type=orderReply&order_id=" + order_id;
	var n = "";
	var w = "600px";
	var h = "500px";
	var s = "resizable:no;help:no;status:no;scroll:yes";
...
function orderShouli(id) {
	...
	<body leftmargin="2" topmargin="2" background='<%=path%>/img/allbg.gif'>
		<table width="98%" border="0" cellpadding="2" cellspacing="1"
			...
			<tr bgcolor="#E7E7E7">
				<td height="14" colspan="44" background="<%=path%>/img/tbg.gif">
					&nbsp;订单管理&nbsp;
				</td>
			</tr>
				...
				<tr align='center' bgcolor="#FFFFFF" height="22">
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${order.zhuangtai=='no'}">
					         未受理&nbsp;&nbsp;&nbsp;<a href="#" style="color: red"
								onclick="orderShouli(${order.id})" class="pn-loperator">受理订单</a>
						</c:if>
						...
						<c:if test="${order.zhuangtai=='ping'}">
												        已评价(<input value="查看" type="button"
								onclick="orderReply(${order.id})" />)
												     </c:if>
					...
					<td>
						<input type="button" onclick="userXinxi(${order.user_id})"
							value="查看会员信息" />
